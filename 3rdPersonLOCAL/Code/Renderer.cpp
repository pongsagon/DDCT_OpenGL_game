...
Renderer::Renderer()
    ...
    , m_lightingShader("Assets/Shaders/lighting.vs", "Assets/Shaders/lighting.fs")
    //, m_lightingShader("Assets/Shaders/1.model_loading.vs", "Assets/Shaders/1.model_loading.fs")
...
void Renderer::RecompileShaders()
{
    ...
    */
    ...
    m_animShader = Shader("Assets/Shaders/anim_model.vs", "Assets/Shaders/anim_model.fs");
}
...
void Renderer::DrawModel(Model& mesh, const glm::vec3& position, const glm::vec3& scale, const glm::vec3& rotation)
{
    glm::mat4 model = glm::translate(glm::mat4(1.0f), position)
        * glm::mat4(glm::quat(rotation))
        * glm::scale(glm::mat4(1.0f), scale);

    m_drawCommands.push_back({ DrawCommandType::Model, model, &mesh });
}

void Renderer::DrawModel(Model& mesh, const glm::mat4& modelMatrix)
{
    m_drawCommands.push_back({ DrawCommandType::Model, modelMatrix, &mesh });
}


void Renderer::DrawAnimation(Model& mesh, const glm::vec3& position, const glm::vec3& scale, const glm::vec3& rotation, const std::vector<glm::mat4>& boneMatrices)
{
    glm::mat4 model = glm::translate(glm::mat4(1.0f), position)
        * glm::mat4(glm::quat(rotation))
        * glm::scale(glm::mat4(1.0f), scale);

    m_drawCommands.push_back({ DrawCommandType::Animation, model, &mesh, boneMatrices });
}

void Renderer::DrawAnimation(Model& mesh, const glm::mat4& modelMatrix, const std::vector<glm::mat4>& boneMatrices)
{
    m_drawCommands.push_back({ DrawCommandType::Animation, modelMatrix, &mesh, boneMatrices });
}
...
void Renderer::Clear()
{
    ...
    glViewport(0, 0, windowSize.x, windowSize.y);
    glClearColor(0.9, 0.95, 1, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void Renderer::RenderScene(Camera& camera, const std::vector<Light>& lights)
{
    Light dirLight;

    if (lights.size() > 0 && lights[0].type == LightType::Directional)
    {
        dirLight.position = lights[0].position;
    }
    ...
    lightView = glm::lookAt(dirLight.position, glm::vec3(0.0f), glm::vec3(0.0, 1.0, 0.0));
    ...
    for (auto& command : m_drawCommands)
    {
        ...
        case DrawCommandType::Cube:
            m_depthShader.setMat4("model", command.model);
            glBindVertexArray(m_cubeVAO);
            glDrawArrays(GL_TRIANGLES, 0, 36);
            glBindVertexArray(0);
            break;
        ...
        case DrawCommandType::Model:
            m_depthShader.setMat4("model", command.model);
            command.mesh->Draw(m_depthShader);
            break;

        case DrawCommandType::Quad:
            m_depthShader.setMat4("model", command.model);
            glBindVertexArray(m_planeVAO);
            glDrawArrays(GL_TRIANGLES, 0, 6);
            break;
    ...
    //glm::mat4 projection = glm::perspective(glm::radians(m_currentCamera->Zoom), (float)windowSize.x / (float)windowSize.y, 0.1f, 100.0f);
    ...
    // set light uniforms
    m_lightingShader.setVec3("viewPos", camera.m_position);
    m_lightingShader.setVec3("lightPos", dirLight.position);
    ...
    for (auto& command : m_drawCommands)
    {
        ...
        case DrawCommandType::Cube:
            m_lightingShader.use();
            m_lightingShader.setMat4("model", command.model);
            m_lightingShader.setVec3("objectColor", command.color);
            m_lightingShader.setInt("textureMode", 0);
            glBindVertexArray(m_cubeVAO);
            glDrawArrays(GL_TRIANGLES, 0, 36);
            glBindVertexArray(0);
            break;
        ...
        case DrawCommandType::Model:
            ...
            m_lightingShader.use();
            m_lightingShader.setMat4("model", command.model);
            m_lightingShader.setInt("textureMode", 1);
            command.mesh->Draw(m_lightingShader);

            */

            m_animShader.use();
            m_animShader.setMat4("projection", camera.GetProjectionMatrix());
            m_animShader.setMat4("view", camera.GetViewMatrix());
            m_animShader.setMat4("model", command.model);
            m_animShader.setBool("animated", false);
            command.mesh->Draw(m_animShader);
        ...
        case DrawCommandType::Animation:
            m_animShader.use();
            m_animShader.setMat4("projection", camera.GetProjectionMatrix());
            m_animShader.setMat4("view", camera.GetViewMatrix());
            m_animShader.setBool("animated", true);

            for (int i = 0; i < command.boneMatrices.size(); ++i)
            {
                m_animShader.setMat4("finalBonesMatrices[" + std::to_string(i) + "]", command.boneMatrices[i]);
            }
